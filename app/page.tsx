...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <main>
        {/* Hero & CTA — Renkli gradient, vurgulu CTA */}
        <section className="min-h-[90vh] flex flex-col items-center justify-center px-6 text-center pt-20 relative overflow-hidden">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight text-white mb-6 max-w-3xl relative">
            Tesisinizi AI Robotlarla Yönetin
          </h1>
          <p className="text-gray-400 max-w-xl mb-10 text-base sm:text-lg relative">
          ...
          <div className="flex flex-col sm:flex-row gap-4 relative">
            ...
            <Button onClick={() => setShowForm(true)} size="lg" variant="outline" className="rounded-xl border-pink-500/40 text-pink-300 hover:bg-pink-500/20 px-8 h-12 font-medium">
              Demo Talep Et
            </Button>
          </div>
        </section>

        {/* Product Features — Renkli kartlar: pembe, mavi, turuncu, yeşil */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {FEATURES.map((f, i) => {
              ...
              <Card key={f.title} className={`bg-gray-900 border ${a.border} rounded-2xl overflow-hidden hover:shadow-lg transition-all`}>
                <CardHeader>
                  <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${a.bg} ${a.text} mb-2`}>
        ...
        <section className="max-w-6xl mx-auto px-6 py-24 border-t border-gray-800 bg-gray-900/50">
            ...
            {REFERANSLAR.map((r, i) => (
              <Card key={i} className="bg-gray-900 border-gray-800 rounded-2xl hover:border-pink-500/30 transition-colors">
                <CardContent className="flex items-center gap-4 p-6">
                  <div className="flex h-12 w-12 items-center justify-center rounded-full bg-pink-500/20 text-pink-400 shrink-0">
          ...
          <div className="mt-10 text-center">
            <Link href="/patron/login">
              <Button className="rounded-xl bg-pink-500 hover:bg-pink-600 text-white px-6 h-11 shadow-lg shadow-pink-500/20">
                Giriş Yap — Deneyin
              </Button>
            </Link>
          </div>
        ...
        {/* Pricing Plans — Renkli paket kartları */}
                  ...
                  <ul className="space-y-3">
                    {pkg.features.map((f, i) => (
                      <li key={i} className="flex items-center gap-2 text-sm text-gray-300">
                        <Check className={`h-4 w-4 shrink-0 ${pkg.highlight ? "text-pink-400" : "text-emerald-400"}`} />
                        {f}
                      </li>
                    ))}
                  </ul>
                ...
                <CardFooter>
                  <Link href="/fiyatlar" className="w-full">
                    <Button
                      ...
                      className={`w-full rounded-xl ${pkg.highlight ? "bg-pink-500 hover:bg-pink-600" : "border-gray-600 text-white hover:bg-gray-800"}`}
                    >
                      Detay
                    </Button>
                  </Link>
        ...
        <section className="max-w-3xl mx-auto px-6 py-20 text-center border-t border-gray-800">
          <Card className="bg-gradient-to-br from-pink-500/10 to-emerald-500/10 border-pink-500/30 rounded-2xl p-8">
            ...
            <Button onClick={() => setShowForm(true)} size="lg" className="rounded-xl bg-amber-500 text-black hover:bg-amber-400 px-8 h-12 font-semibold">
          ...
          <div className="max-w-6xl mx-auto px-6">
            <div className="flex flex-col md:flex-row items-center justify-between gap-8">
              ...
              <div className="flex gap-8 text-sm">
                <Link href="/vitrin" className="text-gray-400 hover:text-pink-400 transition-colors">Vitrin</Link>
                <Link href="/fiyatlar" className="text-gray-400 hover:text-pink-400 transition-colors">Fiyatlar</Link>
