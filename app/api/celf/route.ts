 ...
 * POST: director_key + command → ilgili AI sağlayıcıları döner / işi tetikler
 * Gerçek AI çağrısı chat/flow veya client tarafında yapılır; bu route sınıflandırma ve yönlendirme bilgisi verir.
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      has_veto: hasVeto,
      message: `Görev ${director?.name ?? directorKey} direktörlüğüne yönlendirildi. AI: ${aiProviders.join(', ')}`,
    })
  } catch (e) {
