...
export async function GET() {
  try {
    ...
    // 1. Veritabanı kontrolü (robots yoksa ceo_tasks, patron_commands dene)
    ...
    if (supabase) {
      try {
        ...
        if (dbCheck.error) {
          status.database = { connected: true, error: 'robots/ceo_tasks tablosu yok - Migration çalıştırın' }
